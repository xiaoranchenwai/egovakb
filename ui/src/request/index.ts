...
const axiosConfig = {
  baseURL: (window.APP_CONFIG?.VITE_API_PATH || '') + '/api',
...
export const postStream: (url: string, data?: unknown) => Promise<Result<any> | any> = (
  ...
  const baseUrl = (window.APP_CONFIG?.VITE_API_PATH || '') + url
...
export const socket = (url: string) => {
  ...
  let uri = protocol + window.location.host + (window.APP_CONFIG?.VITE_STATIC_PATH || '') + url
 ...
 * @param data 添加数据
 * @param params 参数
 * @param onProgress 上传进度回调函数 (percentage: number) => void
 * @param loading loading
 * @param timeout 超时时间
 * @returns 异步promise对象
 */
export const postWithProgress: (
  url: string,
  data?: unknown,
  params?: unknown,
  ...
  loading?: NProgress | Ref<boolean>,
  timeout?: number
) => Promise<Result<any> | any> = (url, data, params, onProgress, loading, timeout) => {
  return promise(
    request({
      url: url,
      method: 'post',
