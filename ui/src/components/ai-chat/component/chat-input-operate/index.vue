<template>
  <div class="ai-chat__operate p-16">
    ...
    <!-- 思考功能控制区域 -->
    <div class="thinking-control mb-8" v-if="props.type === 'debug-ai-chat'">
      <el-switch
        ...
        class="mb-2"
      />
      <div v-if="enableThinking" class="mt-2 ml-2">
        <el-radio-group v-model="showThinking">
          <el-radio :label="true">{{ $t('展示思考内容') }}</el-radio>
          <el-radio :label="false">{{ $t('不展示思考内容') }}</el-radio>
        </el-radio-group>
      </div>
    </div>
    <div class="operate-textarea">
      ...
      <div class="flex" :style="{ alignItems: isMicrophone ? 'center' : 'end' }">
        ...
        <el-input
          ...
          "
          :autosize="{ minRows: 5, maxRows: isMobile ? 4 : 5 }"
          type="textarea"
          :maxlength="100000"
...
const loadChatIdFlag = ref<boolean>(false);
...
const handluploadTaskList = async() => {
  if (loadChatIdFlag.value) return
  if (!chatId_context.value) {
    loadChatIdFlag.value = true
    const res = await props.openChatId()
    chatId_context.value = res
    await nextTick()
    loadChatIdFlag.value = false;
    handluploadTaskList()
    return
  }
  for (const item of uploadTaskList.value) {
    if (item.uploadFlag) continue;
...
const uploadFile = async (file: any, fileList: any) => {
  ...
  uploadTaskList.value.push({ uploadFlag: false, data: formData })
  fileList.splice(0, fileList.length)
  handluploadTaskList()
}
...
function autoSendMessage() {
  ...
  if (enableThinking.value) {
    Object.assign(params, { thinking: showThinking.value })
  }
  
  props.sendMessage(inputValue.value, params)
...
.thinking-control {
  ...
  padding: 10px;
  border-radius: 8px;
  box-shadow: 0px 2px 8px 0px rgba(31, 35, 41, 0.08);
}
