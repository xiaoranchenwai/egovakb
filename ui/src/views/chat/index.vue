<template>
  <component v-if="chat_show && init_data_end" :applicationAvailable="applicationAvailable" :is="currentTemplate"
    :application_profile="application_profile" :key="route.fullPath" v-loading="loading" />
  <Auth v-else :application_profile="application_profile" :auth_type="application_profile.authentication_type"
    v-model="is_auth" :style="{
...
onBeforeMount(() => {
  user.changeUserType(2, accessToken||access_token)
  Promise.all([user.asyncGetProfile(), getAccessToken(accessToken||access_token)])
    .catch(() => {
