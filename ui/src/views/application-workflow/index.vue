<template>
  <div class="application-workflow custom-app-workflow" v-loading="loading">
    <div class="header border-b flex-between p-12-24">
      <div class="flex align-center">
        <back-button @click="back"></back-button>
        <h4>{{ detail?.name }}</h4>
        <div v-if="showHistory && disablePublic">
          <el-text type="info" class="ml-16 color-secondary " style="color: #6B7A99"
            >{{ $t('views.applicationWorkflow.info.previewVersion') }}
            {{ currentVersion.name || datetimeFormat(currentVersion.update_time) }}</el-text
          >
        </div>
        <el-text type="info" class="ml-16 color-secondary" v-else-if="saveTime" style="color: #6B7A99"
      ...
      <div v-else>
        ...
        <el-button type="primary" @click.stop='onViewChat'>
        ...
        <div class="workflow-debug-header" :class="!isDefaultTheme ? 'custom-header' : ''">
          <div class="flex-between">
            <div class="flex align-center">
              ...
              <h4 style='color: var(--app-header-text-color)'>
            ...
            <div class="mr-16">
              <el-button link @click="enlarge = !enlarge">
                <AppIcon :iconName="enlarge ? 'app-minify' : 'app-magnify'" class="color-secondary"
                  style="font-size: 20px; color:var(--app-header-text-color)">
                </AppIcon>
              </el-button>
              <el-button link @click="showDebug = false">
                <el-icon :size="20" class="color-secondary">
                  <Close style='color:var(--app-header-text-color)'/>
                </el-icon>
              </el-button>
            </div>
          </div>
