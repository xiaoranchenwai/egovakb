<template>
  <div class="common-table-view">
    <LayoutContainer :header="$t('common.fileUpload.document')">
      <div>
      <div class="table">
        <div class="condition flex flex-between">
          <div>
            <el-button v-if="datasetDetail.type === '0'" round
              @click="router.push({ path: '/dataset/upload', query: { id: id } })"><el-icon><Upload /></el-icon>{{ $t('views.document.uploadDocument') }}</el-button>
            <el-button v-if="datasetDetail.type === '1'" round @click="importDoc">
              <el-icon><Upload /></el-icon>
              {{$t('views.document.importDocument')}}
            </el-button>
            <el-button @click="syncMulDocument" :disabled="multipleSelection.length === 0" round
              v-if="datasetDetail.type === '1'">
              <el-icon><Refresh /></el-icon>{{ $t('views.document.syncDocument') }}
            </el-button>
            <el-button @click="openDatasetDialog()" :disabled="multipleSelection.length === 0" round>
              <el-icon><Right /></el-icon>{{ $t('views.document.setting.migration') }}
            </el-button>
            <el-button @click="batchRefresh" :disabled="multipleSelection.length === 0" round>
              <el-icon><Switch /></el-icon>{{ $t('views.dataset.setting.vectorization') }}
            </el-button>
            <el-button @click="openGenerateDialog()" :disabled="multipleSelection.length === 0" round>
              <el-icon><Connection /></el-icon>{{ $t('views.document.generateQuestion.title') }}
            </el-button>
            <el-button @click="openBatchEditDocument" :disabled="multipleSelection.length === 0" round>
              <el-icon><Setting /></el-icon>{{ $t('common.setting') }}
            </el-button>
            <el-button @click="deleteMulDocument" :disabled="multipleSelection.length === 0" round>
              <el-icon><Delete /></el-icon>{{ $t('common.delete') }}
            </el-button>
            <el-button @click="getList" round>
              <el-icon><Refresh /></el-icon>{{ $t('common.refresh') }}
            </el-button>
          </div>

          <el-input v-model="filterText" placeholder="按 文档名称 搜索" suffix-icon="Search" class="w-240" @change="getList"
            clearable />
        </div>

        <app-table ref="multipleTableRef" :data="documentData" :pagination-config="paginationConfig"
            ...
            <template #default="{ row }">
              <div class="flex items-center">
                ...
                <div v-if="row.document_url">
                  <el-tooltip :content="row.document_url" placement="top">
                    <el-icon @click.stop="openLink(row.document_url)" style="cursor: pointer;color: #409EFF;font-size: 15px">
                      <Link />
                    </el-icon>
                  </el-tooltip>
                </div>
                ...
                <el-dropdown trigger="click" @command="dropdownHandle">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu style="width: 100px">
                      ...
                      <el-dropdown-item
                        :class="filterMethod['status'] === State.REVOKE ? 'is-active' : ''"
                        class="justify-center"
                        :command="beforeCommand('status', State.REVOKE)"
                        >{{ $t('views.document.fileStatus.REVOKE') }}</el-dropdown-item
                      >
                      <el-dropdown-item
                        :class="
                          filterMethod['status'] === State.STARTED &&
            ...
            <template #header>
              <div>
                ...
                <el-dropdown trigger="click" @command="dropdownHandle">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu style="width: 100px">
                      <el-dropdown-item :class="filterMethod['hit_handling_method'] ? '' : 'is-active'"
                        :command="beforeCommand('hit_handling_method', '')" class="justify-center">{{$t('views.document.table.all')}}</el-dropdown-item>
                      <template v-for="(value, key) of hitHandlingMethod" :key="key">
                        <el-dropdown-item :class="filterMethod['hit_handling_method'] === key ? 'is-active' : ''"
                          class="justify-center" :command="beforeCommand('hit_handling_method', key)">{{ $t(value)
                          }}</el-dropdown-item>
                      </template>
                    </el-dropdown-menu>
                  </template>
          ...
          <el-table-column :label="$t('common.operation')" align="left" width="150" fixed="right">
            <template #default="{ row }">
...
.table {
  :deep(.app-table) {
    .el-table__header {
      ...
      .caret-wrapper {
        .sort-caret {
          --el-text-color-placeholder: #A8B4C8;
          &.ascending {
            border-bottom-color: #A8B4C8;
          }
          &.descending {
            border-top-color: #A8B4C8;
          }
        }
      }
    }
