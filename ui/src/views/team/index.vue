<template>
  <LayoutContainer :header="$t('views.team.title')" class="team-page">
    <template #header>
      <div class="flex-between w-full">
        ...
        <div>
          ...
          <el-button type="primary" @click="submitPermissions" :disabled="loading">
            保存
          </el-button>
        </div>
    ...
    <div class="team-manage flex main-calc-height">
      <div class="team-member border-r">
      ...
      <div class="permission-setting flex" v-loading="rLoading">
        <div class="team-manage__table">
          <h4 class="p-20 pb-0 mb-4">{{ $t('views.team.permissionSetting') }}</h4>
          <el-tabs v-model="activeName" class="team-manage__tabs">
            <el-tab-pane v-for="(item, index) in settingTags" :key="item.value" :label="item.label" :name="item.value">
              <PermissionSetting :key="index" :data="item.data" :type="item.value" :tableHeight="tableHeight"
                :manage="isManage(currentType)"></PermissionSetting>
            </el-tab-pane>
          </el-tabs>
        </div>
...
.team-page {
  ...
  .el-tabs .el-tabs__header .el-tabs__nav-scroll .el-tabs__nav {
