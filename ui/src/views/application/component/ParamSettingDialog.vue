<template>
  <el-dialog align-center :title="$t('common.paramSetting')" class="param-dialog" v-model="dialogVisible"
    style="width: 600px" append-to-body :close-on-click-modal="false" :close-on-press-escape="false">
      ...
      <div class="p-16">
        <el-form label-position="top" ref="paramFormRef" :model="form">
          <el-form-item :label="$t('views.application.applicationForm.dialog.selectSearchMode') + ':'">
            <el-radio-group v-model="form.dataset_setting.search_mode" class="card__radio" @change="changeHandle">
              <el-card shadow="never" class="mb-16" :class="form.search_mode === 'embedding' ? 'active' : ''">
                <el-radio value="embedding" size="large">
                  <p class="mb-4 g60">
                    {{ $t('views.application.applicationForm.dialog.vectorSearch') }}
                  </p>
                  <el-text type="info" class="g50">{{
              ...
              <el-card shadow="never" :class="form.dataset_setting.search_mode === 'blend' ? 'active' : ''">
                ...
                <template #label>
                  <div class="flex align-center">
                    ...
                    <el-tooltip effect="dark"
                      :content="$t('views.application.applicationForm.dialog.similarityTooltip')" placement="right">
                      <AppIcon iconName="app-warning" class="app-warning-icon"></AppIcon>
                    </el-tooltip>
                  </div>
                </template>
                <el-input-number v-model="form.dataset_setting.similarity" :min="0"
                  :max="form.search_mode === 'blend' ? 2 : 1" :precision="3" :step="0.1" :value-on-clear="0"
                  controls-position="right" class="w-full text-left" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item :label="$t('views.application.applicationForm.dialog.topReferences') + ':'">
                <el-input-number v-model="form.dataset_setting.top_n" :min="1" :max="10000" :value-on-clear="1"
                  controls-position="right" class="w-full text-left" />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-form-item :label="$t('views.application.applicationForm.dialog.maxCharacters') + ':'">
            <el-slider v-model="form.dataset_setting.max_paragraph_char_number" show-input :show-input-controls="false"
              :min="500" :max="100000" class="custom-slider" />
          </el-form-item>

          <el-form-item v-if="!isWorkflowType"
            :label="$t('views.application.applicationForm.dialog.noReferencesAction') + ':'">
            <el-form label-position="top" ref="noReferencesformRef" :model="noReferencesform" :rules="noReferencesRules"
              :hide-required-asterisk="true" class="w-full">
              <el-radio-group v-model="form.dataset_setting.no_references_setting.status" class="radio-block-avatar">
                ...
                <el-radio value="designated_answer">
                  ...
                  <el-form-item v-if="form.dataset_setting.no_references_setting.status === 'designated_answer'"
                    prop="designated_answer">
                    <el-input v-model="noReferencesform.designated_answer" :rows="2" type="textarea" maxlength="2048"
          ...
          <el-form-item @click.prevent v-if="!isWorkflowType">
            ...
            <el-switch v-model="form.problem_optimization" active-text="是" inactive-text="否" inline-prompt ></el-switch>
          </el-form-item>
          <el-form-item v-if="form.problem_optimization"
            :label="$t('views.application.applicationForm.form.prompt.label') + ':'">
            <el-input v-model="form.problem_optimization_prompt" :rows="6" type="textarea" maxlength="2048"
...
.param-dialog {
  ...
  .el-card {
    --el-card-border-color: #DDE1EB;
  }

  :deep(.el-scrollbar) {
    --el-scrollbar-bg-color: #A8B4C8;
