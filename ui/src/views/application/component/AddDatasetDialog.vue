<template>
  ...
  >
    <template #header="{ titleId, titleClass }">
      <div class="flex-between w-full align-center" style="height: 100%;">
        <h4 :id="titleId" :class="titleClass">
          {{ $t('views.application.applicationForm.dialog.addDataset') }}
        </h4>
        <div class="flex align-center mr-8">
          <el-button link class="ml-16 g60" @click="refresh">
            <el-icon class="mr-4"><Refresh /></el-icon>{{ $t('common.refresh') }}
          </el-button>
          <el-divider direction="vertical" class="g60"/>
        </div>
      </div>
      
    </template>
    <el-scrollbar>
      <div class="flex-between ml-16 mr-16 mb-16">
    ...
    <template #footer>
      <div class="flex-between w-full">
        <div class="flex">
          <el-text type="info" class="color-secondary mr-8 g50" v-if="checkList.length > 0">
...
.addDataset-dialog {
  ...
  .el-dialog__footer {
    padding-left: var(--app-view-padding);
  }
  .max-height {
