<template>
  <el-dialog
    ...
    :before-close="close"
    append-to-body
    modal-class="template-create-modal"
  >
    ...
    <el-tabs v-model="activeName">
      ...
      >
        ...
        <div class="flex-between mb-8">
          <h5>{{ $t('views.template.templateForm.title.modelHeaders') }}</h5>
          <el-button type="text" @click.stop="openAddHeaderDrawer()">
            <AppIcon iconName="Plus" class="add-icon" />{{ $t('common.add') }}
          </el-button>
        </div>
        <el-table :data="base_form_data.http_header" class="mb-16">
          <el-table-column prop="name" :label="$t('views.template.templateForm.header.name')" show-overflow-tooltip />
          <el-table-column prop="value" :label="$t('views.template.templateForm.header.value')" show-overflow-tooltip />
          <el-table-column :label="$t('common.operation')" align="left" width="90">
            <template #default="{ row, $index }">
              <span class="mr-4">
                <el-tooltip effect="dark" :content="$t('common.modify')" placement="top">
                  <el-button type="primary" text @click.stop="openAddHeaderDrawer(row, $index)">
                    <el-icon><EditPen /></el-icon>
                  </el-button>
                </el-tooltip>
              </span>
              <el-tooltip effect="dark" :content="$t('common.delete')" placement="top">
                <el-button type="primary" text @click="deleteHeader($index)">
  ...
  <AddHeaderDrawer ref="AddHeaderRef" @refresh="refreshHeader" />
</template>
...
import AddHeaderDrawer from '@/views/template/component/AddHeaderDrawer.vue'
...
const base_form_data = ref<{
  ...
  http_header: any[]
}>({ name: '', model_type: '', model_name: '', permission_type: 'PRIVATE', model_params_form: [], http_header: [] })
...
const form_data = computed({
  get: () => {
    return {
      ...
      http_header: base_form_data.value.http_header
    }
  },
  set: (event: any) => {
...
function openAddHeaderDrawer(data?: any, index?: any) {
  AddHeaderRef.value?.open(data, index)
}

function deleteHeader(index: number) {
  base_form_data.value.http_header.splice(index, 1)
}
...
function refreshHeader(data: any, index: any) {
  for (let i = 0; i < base_form_data.value.http_header.length; i++) {
    if (data.name === base_form_data.value.http_header[i].name && index !== i) {
      MsgError(t('views.template.tip.errorMessage') + data.name)
      return
    }
  }
  if (index !== null) {
    base_form_data.value.http_header.splice(index, 1, data)
  } else {
    base_form_data.value.http_header.push(data)
  }
}
