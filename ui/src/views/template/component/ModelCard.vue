    ...
    <template #header>
      <div class="flex">
        ...
        <div style="width: calc(100% - 32px - 4px - var(--app-base-px))">
          ...
          <div class="mt-4">
            <el-tag v-if="model.permission_type === 'PRIVATE'" type="danger" class="danger-tag g50">{{
              $t('common.private')
            }}</el-tag>
            <el-tag v-else type="info" class="info-tag g50"> {{ $t('common.public') }}</el-tag>
          </div>
        </div>
      </div>
    </template>

    <div class="mt-16 infos">
      <ul>
        <li class="flex mt-16">
          <el-text type="info" class='g50'>{{
            $t('views.template.templateForm.form.model_type.label')
          }}</el-text>
          <span class="ellipsis ml-16 g60">
        ...
        <li class="flex mt-12">
          ...
          <span class="ellipsis-1 ml-16 g60" style="height: 20px; width: 70%">
            {{ model.model_name }}</span
          >
        </li>
        <li class="flex mt-12">
          <el-text type="info" class='g50'>{{ $t('common.creator') }}</el-text>
          <span class="ellipsis-1 ml-16 g60" style="height: 20px; width: 70%">
            {{ model.username }}</span
          >
        </li>
...
const is_permisstion = computed(() => {
  return user.userInfo?.id == props.model.user_id || user.userInfo?.role === 'ADMIN'
})
  ...
  MsgConfirm(
    ...
    .then(() => {
      ModelApi.deleteModel(props.model.id).then(() => {
        ...
        if (error && error.message && error.message.includes('该模型关联了')) {
          MsgConfirm(
            t('views.template.delete.confirmTitle'),
            `${error.message}\n${t('views.template.delete.confirmForceDelete')}`,
            {
              confirmButtonText: t('common.confirmDelete'),
              confirmButtonClass: 'danger'
            }
          ).then(() => {
            ModelApi.deleteModel(props.model.id, true).then(() => {
              emit('change')
            })
          }).catch(() => {})
        } else {
          MsgError(error.message || t('common.operationFailed'))
        }
      })
    })
    .catch(() => {})
