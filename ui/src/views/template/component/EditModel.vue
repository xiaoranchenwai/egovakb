<template>
  <el-dialog v-model="dialogVisible" width="680px" :close-on-click-modal="false" :close-on-press-escape="false"
    :destroy-on-close="true" :before-close="close" modal-class="template-edit-modal">
    ...
    <DynamicsForm v-loading="formLoading" v-model="form_data" :render_data="model_form_field" :model="form_data"
      ref="dynamicsFormRef" label-position="left" require-asterisk-position="left" label-width="8em">
      <template #default>
        <el-form-item prop="name" :rules="base_form_data_rule.name">
          <template #label>
            <div class="flex align-center" style="display: inline-flex">
              <div class="mr-4">
                <el-text truncated>
                  {{ $t('views.template.templateForm.form.templateName.label') }}
                </el-text>
              </div>
          ...
          <el-radio-group v-model="base_form_data.permission_type" class="card__radio">
            <el-row :gutter="16">
              <template v-for="(value, key) of PermissionType" :key="key">
                <el-col :span="12">
                  <el-card shadow="never" class="mb-16" :class="base_form_data.permission_type === key ? 'active' : ''">
