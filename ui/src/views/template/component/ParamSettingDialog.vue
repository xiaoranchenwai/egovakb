<template>
  ...
  >
    ...
    <div class="mt-24">
      <div class="mb-12">
        <el-button type="primary" @click="openAddHeaderDrawer()">
          {{ $t('views.template.templateForm.title.addHeader') }}
        </el-button>
      </div>
      <el-table :data="headerParams" class="mb-16">
  ...
  <AddHeaderDrawer ref="AddHeaderRef" :model-id="model.id" @refresh="refreshHeader" />
</template>
...
import AddHeaderDrawer from './AddHeaderDrawer.vue'
...
const open = () => {
  ...
  Promise.all([
    ...
    ModelApi.getModelHeaders(props.model.id, loading)
  ]).then(([paramsRes, headersRes]) => {
    loading.value = false
    modelParamsForm.value = paramsRes.data
    headerParams.value = headersRes.data
  }).catch(() => {
    loading.value = false
  })
}
...
function refreshHeader(data: any, index: any) {
  for (let i = 0; i < headerParams.value.length; i++) {
    if (data.name === headerParams.value[i].name && index !== i) {
      MsgError(t('views.template.tip.errorMessage') + data.name)
      return
    }
  }
  if (index !== null) {
    headerParams.value.splice(index, 1, data)
  } else {
...
function submit() {
  Promise.all([
    ModelApi.updateModelParamsForm(props.model.id, modelParamsForm.value, loading),
    ModelApi.updateModelHeaders(props.model.id, headerParams.value, loading)
  ]).then(() => {
