<template>
  <login-layout v-if="!loading" v-loading="loading">
    <LoginContainer :subTitle="user.themeInfo?.slogan || $t('views.system.theme.defaultSlogan')">
      <h2 class="mb-24" v-if="!showQrCodeTab" style="font-weight: 700;">{{ loginMode || $t('views.login.title') }}</h2>
...
onBeforeMount(() => {
  console.log('onBeforeMount, login')
  loading.value = true
...
.login-card {
  box-shadow: 0 8px 24px 0 #124ab429 !important;
