<template>
  <el-dialog :title="$t('views.problem.relateParagraph.title')" v-model="dialogVisible" width="80%" class="paragraph-dialog" destroy-on-close
    :close-on-click-modal="false" :close-on-press-escape="false" modal-class="relate-problem-dialog">
    <el-row v-loading="loading">
      <el-col :span="6">
        <el-scrollbar height="500" wrap-class="paragraph-scrollbar">
          <div class="bold title align-center pb-0 flex" style="padding: 10px">
            <div style="display: flex;width: 5em;min-width: 5em;">{{$t('views.problem.relateParagraph.selectDocument')}}</div>
            <el-input v-model="filterDoc" :placeholder="$t('views.problem.relateParagraph.placeholder')" suffix-icon="Search" clearable />
          </div>
          <div class="p-8" style="padding-bottom: 8px">

            <common-list :data="documentList" class="mt-8" @click="clickDocumentHandle"
              ...
              <template #default="{ row }">
                <span class="flex lighter align-center">
                  ...
                  <el-badge :value="associationCount(row.id)" type="primary" v-if="associationCount(row.id)"
      ...
      <el-col :span="18" class="border-l">
        <el-scrollbar height="500" wrap-class="paragraph-scrollbar">
          <div class="p-24" style="padding-bottom: 8px; padding-top: 16px">
            ...
            <InfiniteScroll v-else :size="paragraphList.length" :total="paginationConfig.total"
              :page_size="paginationConfig.page_size" v-model:current_page="paginationConfig.current_page"
              @load="getParagraphList" :loading="loading">
              <template v-for="(item, index) in paragraphList" :key="index">
                <CardBox shadow="hover" :title="item.title || '-'" :description="item.content"
                  class="paragraph-card cursor mb-16" :class="isAssociation(item.id) ? 'selected' : ''"
                  :showIcon="false" @click="associationClick(item)">
                </CardBox>
              </template>
            </InfiniteScroll>
          </div>
