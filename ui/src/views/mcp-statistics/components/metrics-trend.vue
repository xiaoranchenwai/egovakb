...
const active = ref<number>(7)
const lineChartRef = ref()
const map: any = {
  total_template_groups: '分类数',
  total_templates: '模板数',
  total_service_calls: '服务数',
  total_tools_calls: '工具总数'
}
...
const createOption = (dataList: Array<any>) => {
  ...
  for (let i = 0, len = keys.length; i < len; i++) {
    const key = keys[i]
    const name = map[key]
    lineOption.series.push(getLineSeriesData(name + '', dataList.map(g => g[key]), i))
  }
  initChart()
}
