...
import { barOptionConfig, DATE_FORMAT, getBarSeriesData } from './options'
import mcpStatisticsApi from '@/api/mcp-statistics'

const { getMcpGroupStat } = mcpStatisticsApi
const active = ref<any>('templates_count')
...
const createOption = (dataList: Array<any>) => {
  const xAxisData = dataList.map(g => g.group_name)
