export default {
  ...
  setRules: {
    ...
    chunk_patterns: {
      label: '子分段标识',
      tooltip: '设置子分段的分割标识，按照所选符号先后顺序做递归分割',
      placeholder: '请选择子分段标识'
    },
    chunk_length: {
      label: '子分段长度',
      tooltip: '设置每个子分段的最大字符长度，超出后会自动分段'
    },
    limit: {
      label: '分段长度'
    },
  ...
  uploadType: {
    tooltip: '简易模式，只需要上传文档，平台自动分段、生成关联问题，适合新手用户；\n自定义模式，需要确认分段后再上传，关联问题需要人工选择分段后自动生成，适合熟练用户调优。',
    generateRelated: '通过大模型，让大模型基于每个文档内容，自动生成数个问题，方便后续检索文段',
    generateRelatedCount: '通过大模型，让大模型基于每个文档内容，自动生成的问题数量，最小值1，最大值6',
    splitLimit: '设置每个分段的字数限制，超出后会自动分段，每个分段最小50字，最大10万字',
    cover: '开启后，会删除之前的同名文档，实现知识库中只有最新上传的文档'
  },
  feishu: {
    selectDocument: '选择文档',
