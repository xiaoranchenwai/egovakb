 ...
 * @returns
 */
const getModelHeaders: (
  model_id: string,
  loading?: Ref<boolean>
) => Promise<Result<Array<{ name: string; value: string }>>> = (model_id, loading) => {
  return get(`model/${model_id}/http_headers`, {}, loading)
}

/**
 * 更新模型 HTTP Header 配置
 * @param model_id 模型id
 * @param headers Header 配置列表
 * @param loading 加载器
 * @returns
 */
const updateModelHeaders: (
  model_id: string,
  headers: Array<{ name: string; value: string }>,
  loading?: Ref<boolean>
) => Promise<Result<boolean>> = (model_id, headers, loading) => {
  return put(`model/${model_id}/http_headers`, headers, {}, loading)
}
...
const deleteModel: (model_id: string, force?: boolean, loading?: Ref<boolean>) => Promise<Result<boolean>> = (
  model_id,
  force = false,
  loading
) => {
  return del(`${prefix}/${model_id}${force ? '?force=true' : ''}`, undefined, {}, loading)
}
export default {
  getModel,
  getProvider,
