 ...
 * @param 参数
 */
const getAllAppilcation: (param: any, loading?: Ref<boolean>) => Promise<Result<any[]>> = (param, loading) => {
...
const getApplicationTree: () => Promise<Result<any>> = (
) => {
  return get(`/application/group/tree`)
}

const searchApplicationTree: (data: any, loading?: Ref<boolean>) => Promise<Result<any>> = (
  data,
  loading
) => {
  return post(`/application/group/tree`, data, loading)
}
