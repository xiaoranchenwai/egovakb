 ...
 */
/**
 * Export dataset as Excel with name: <dataset_name>_<YYYYMMDDHHmmss>.xlsx
 * 导出知识库为 Excel，文件名采用：<知识库名称>_<YYYYMMDDHHmmss>.xlsx
 * - Preserves Chinese characters in file name
 * - Sanitizes Windows-illegal characters: \ / : * ? " < > |
 */
const exportDataset: (
  dataset_name: string,
  dataset_id: string,
  ...
  const safeName = dataset_name.replace(/[\\/:*?"<>|]/g, '_')
  const fileName = `${safeName}_${ts}.xlsx`
  return exportExcel(fileName, `dataset/${dataset_id}/export`, undefined, loading)
}

/**
 * Import dataset from an exported Excel file.
 * 通过导出的 Excel 文件导入知识库。
 * - Sends multipart form data with key `file`.
 * - Backend accepts keys: `file` | `dataset` | `dataset_file`.
 */
const importDataset: (
  dataset_id: string,
  file: File,
  loading?: Ref<boolean>
) => Promise<Result<any>> = (dataset_id, file, loading) => {
  const form = new FormData()
  form.append('file', file)
  return post(`${prefix}/${dataset_id}/import`, form, undefined, loading)
}

/**
 * Import to a NEW dataset when no target is selected.
 * - Backend will create a dataset named: dataset.name + import timestamp.
 */
const importDatasetNew: (
  file: File,
  loading?: Ref<boolean>
) => Promise<Result<any>> = (file, loading) => {
  const form = new FormData()
  form.append('file', file)
  return post(`${prefix}/import`, form, undefined, loading)
}
/**
 *导出Zip知识库
 * @param dataset_name 知识库名称
