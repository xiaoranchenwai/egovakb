...
class WebLocalEmbedding(MaxKBBaseModel, BaseModel, Embeddings):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        if len(CONFIG.get("BACKEND_API_PREFIX")) > 0:
            backend_prefix = CONFIG.get("BACKEND_API_PREFIX").rstrip('/')
            bind = f'{bind}/{backend_prefix}'
...
class LocalEmbedding(MaxKBBaseModel, HuggingFaceEmbeddings):
    ...
    def new_instance(model_type, model_name, model_credential: Dict[str, object], **model_kwargs):
        if model_kwargs.get('use_local', True):
            if model_credential.get('device') is None or len(model_credential.get('device')) == 0:
                model_credential['device'] = 'cpu'
