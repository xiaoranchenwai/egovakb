...
class IModelProvider(ABC):
    ...
    def is_valid_credential(self, model_type, model_name, model_credential: Dict[str, object],
                            model_params: Dict[str, object], model_headers: Dict[str, object], raise_exception=False):
        model_info = self.get_model_info_manage().get_model_info(model_type, model_name)
        if model_headers is not None and model_headers != {}:
            return model_info.model_credential.is_valid(model_type, model_name, model_credential, model_params, self,
                                                    raise_exception=raise_exception, model_headers=model_headers)
        else:
            return model_info.model_credential.is_valid(model_type, model_name, model_credential, model_params, self,
                                                    raise_exception=raise_exception)

    def get_model_v2(self, model_type, model_name, model_credential: Dict[str, object], model_headers, **model_kwargs) -> BaseModel:
        model_info = self.get_model_info_manage().get_model_info(model_type, model_name)
        try:
            return model_info.model_class.new_instance(model_type, model_name, model_credential, model_headers=model_headers, **model_kwargs)
        except Exception as e:
            # 捕获模型连接错误并返回详细信息
            error_msg = f"Model connection error for {model_name} (v2): {str(e)}"
    ...
    def get_model(self, model_type, model_name, model_credential: Dict[str, object], **model_kwargs) -> BaseModel:
        ...
        try:
            ...
            # 捕获模型连接错误并返回详细信息
            error_msg = f"Model connection error for {model_name}: {str(e)}"
            raise Exception(error_msg) from e
