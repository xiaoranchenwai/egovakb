...
class ModelSerializer(serializers.Serializer):
    ...
    class Create(serializers.Serializer):
        ...
        http_header = serializers.ListField(required=False, default=list,
                                                  error_messages=ErrMessage.char(_("http header configuration")))
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            default_headers = {item['name']: item['value'] for item in self.data.get('http_header')} if self.data.get('http_header') is not None else None
        ...
        def insert(self, user_id, with_valid=False):
            ...
            http_header = self.data.get('http_header')
    ...
    class ModelHttpHeaders(serializers.Serializer):
        ...
        def get_model_headers(self, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            model_id = self.data.get('id')
            model = QuerySet(Model).filter(id=model_id).first()
            return model.http_header

        def save_model_headers(self, headers, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            if headers is None:
                headers = []
    ...
    def model_to_dict(model: Model):
        try:
            decrypted_credential = rsa_long_decrypt(model.credential)
            if not decrypted_credential or decrypted_credential.strip() == '':
                raise ValueError(f"解密后的凭证为空，模型ID: {model.id}，可能是数据库恢复后凭证数据损坏")
            credential = json.loads(decrypted_credential)
        except json.JSONDecodeError as e:
            raise ValueError(f"凭证JSON解析失败，模型ID: {model.id}，解密内容: '{decrypted_credential[:100] if 'decrypted_credential' in locals() else 'N/A'}'，错误: {str(e)}")
        except Exception as e:
            raise ValueError(f"凭证处理失败，模型ID: {model.id}，错误: {str(e)}")
    ...
    class Operate(serializers.Serializer):
        ...
        role = serializers.CharField(required=False, error_messages=ErrMessage.char(_("user role")))
        
        force = serializers.BooleanField(required=False, default=False, error_messages=ErrMessage.boolean(_("force delete")))
        ...
        def delete(self, with_valid=True):
            ...
            if self.data.get('force'):
                max_kb.info(f"强制删除模型：{model_id}，{model.name}，关联应用：{','.join([application.name for application in Application.objects.filter(model_id=model_id)])}")
                model.delete()
                return True            
            if model.model_type == 'LLM':
                applications = Application.objects.filter(model_id=model_id).all()
                if len(applications) > 0:
                    application_names = [application.name for application in applications]                    
                    raise AppApiException(500, f"该模型关联了{len(applications)} 个应用，无法删除该模型。关联应用：{','.join(application_names)}")
        ...
        def edit(self, instance: Dict, user_id: str, with_valid=True):
            ...
            if model is None:
                ...
                try:
                    ...
                    default_headers = {item['name']: item['value'] for item in model.http_header} if model.http_header is not None else None
                    # 校验模型认证数据
