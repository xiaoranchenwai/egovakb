...
from common.constants.permission_constants import Operate, RoleConstants
...
class UpdateTeamMemberPermissionSerializer(ApiMixin, serializers.Serializer):
    ...
    def is_valid(self, *, user_id=None, role=None):
...
class TeamMemberSerializer(ApiMixin, serializers.Serializer):
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def list_member_permission(self, with_valid=True):
            ...
            if self.data.get('role') == RoleConstants.ADMIN.name:
                member_permission_list = select_list(
                    get_file_content(os.path.join(PROJECT_DIR, "apps", "setting", 'sql', 'get_member_permission_admin.sql')),
                    [])
            else:
                member_permission_list = select_list(
        ...
        def edit(self, member_permission: Dict):
            ...
            #     pass
            # else:
            s.is_valid(user_id=self.data.get("team_id"), role=self.data.get('role'))
