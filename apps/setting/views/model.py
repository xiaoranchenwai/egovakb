...
class Model(APIView):
    ...
    class ModelHttpHeaders(APIView):
        authentication_classes = [TokenAuth]
        ...
        def get(self, request: Request, model_id: str):
            return result.success(
                ModelSerializer.ModelHttpHeaders(data={'id': model_id, 'user_id': request.user.id}).get_model_headers())

        @action(methods=['PUT'], detail=False)
        @has_permissions(PermissionConstants.MODEL_READ)
        def put(self, request: Request, model_id: str):
            return result.success(
                ModelSerializer.ModelHttpHeaders(data={'id': model_id, 'user_id': request.user.id})
                .save_model_headers(request.data))
