...
class FunctionLibSerializer(serializers.Serializer):
    class Query(serializers.Serializer):
        ...
        def page(self, current_page: int, page_size: int, with_valid=True):
            ...
            function_list = page_search(current_page, page_size, self.get_query_set(),
                               post_records_handler=post_records_handler)
            # user_list = QuerySet(User).filter(id__in=[item.get('user_id') for item in function_list])
            if function_list.get('records'):
                # 获取所有用户ID
                user_ids = [func.get('user_id') for func in function_list.get('records')]
                ...
                # 为每个应用添加用户名和头像
                for func in function_list.get('records'):
                    user_id = func.get('user_id')
                    if str(user_id) in users:
                        func['username'] = users[str(user_id)].username
                    if str(user_id) in user_settings:
                        func['user_avatar'] = user_settings[str(user_id)].value
