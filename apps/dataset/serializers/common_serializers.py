...
def get_embedding_model_by_dataset_id(dataset_id: str):
    ...
    try:
        ...
        # 捕获模型连接错误并返回详细信息
        model_name = getattr(dataset.embedding_mode, 'model_name', 'Unknown Model')
        provider = getattr(dataset.embedding_mode, 'provider', 'Unknown Provider')
        
        # 检查是否为JSON解析错误
        if "Expecting value: line 1 column 1 (char 0)" in str(e):
            error_msg = f"Model connection error for {model_name} ({provider}): The model service returned an empty or invalid response. Please check if the model service is running and accessible."
        else:
            error_msg = f"Model connection error for {model_name} ({provider}): {str(e)}"
