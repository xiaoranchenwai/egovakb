...
from common.constants.dataset_constants import DatasetUploadConsts
...
from dataset.models.data_set import Chunk, DataSet, Document, Paragraph, Problem, Type, ProblemParagraphMapping, Image, \
    TaskType, State
...
class DocumentSerializers(ApiMixin, serializers.Serializer):
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def refresh(self, state_list=None, with_valid=True):
            ...
            try:
                max_kb.info(f"embedding_by_document.delay dataset:{dataset.name} document:{document.name}")
    ...
    class Create(ApiMixin, serializers.Serializer):
        ...
        @staticmethod
        def import_and_generate_questions(documents_data: list, dataset, request: Request):
            """生成文档相关问题
            ...
            Args:
                ...
                request: 请求对象
            """
            generateRelated = request.data.get('generateRelated')
            if generateRelated == DatasetUploadConsts.GENERATE_RELATED.value and len(documents_data) > 0:
                generateRelatedCount = int(request.data.get('generateRelatedCount'))
                if generateRelatedCount < 1 or generateRelatedCount > 6:
                    raise AppApiException(500, _('The number of questions generated per segment must be between 1 and 6'))                
                if dataset is not None and dataset['question_model_id'] is not None:
                    generate_related_data = {
                        "model_id": str(dataset['question_model_id']),
                        "prompt": f"内容：{{data}}\n\n请总结上面的内容，并根据内容总结生成 {generateRelatedCount} 个问题。\n回答要求：\n- 请只输出问题；\n- 请将每个问题放置<question></question>标签中。",
                        "document_id_list": [],
                        "state_list": [
                            "0", "1", "2", "3", "4", "5", "n"
                        ]
                    ...
                    for doc in documents_data:
                        generate_related_data['document_id_list'].append(doc['id'])
                    DocumentSerializers.BatchGenerateRelated(
                        data={'dataset_id': dataset['id']}
                    ).batch_generate_related(generate_related_data)
        ...
        def save_table(self, instance: Dict, request: Request, dataset, with_valid=True):
            if with_valid:
            ...
            cover = request.data.get('cover', 'false').lower() == 'true'
            if cover:
                names = [doc['name'] for doc in document_list]
                if len(names) > 0:
                    QuerySet(Document).filter(name__in=names).delete()
            documents_data = DocumentSerializers.Batch(data={'dataset_id': self.data.get('dataset_id')}).batch_save(document_list)
            DocumentSerializers.Create.import_and_generate_questions(documents_data, dataset, request)
        ...
        def get_paragraph_model(document_model, paragraph_list: List):
            ...
            for paragraphs in paragraph_model_dict_list:
                ...
                chunk_model_list.extend(paragraphs.get('chunk_model_list'))
            ...
            return {'document': document_model, 'paragraph_model_list': paragraph_model_list,
                    ...
                    'chunk_model_list': chunk_model_list}
        ...
        def get_document_paragraph_model(dataset_id, instance: Dict):
            document_model = Document(
                **{'dataset_id': dataset_id,
                   ...
                   'document_url': instance.get('document_url') if instance.get('document_url') is not None else ''})
    ...
    class Split(ApiMixin, serializers.Serializer):
        ...
        def parse(self):
            ...
            parsed_data = reduce(lambda x, y: [*x, *y],
    ...
    class SplitPattern(ApiMixin, serializers.Serializer):
        ...
        def list():
            return [
                    {'key': _('空行(blank line)'), 'value': '(?<!\\n)\\n\\n(?!\\n)', 'description': '匹配单个空行（两个连续换行符，排除三个及以上）', 'recommendedLevel': 5},
                    {'key': _('换行(enter)'), 'value': '(?<!\\n)\\n(?!\\n)', 'description': '匹配单个换行符（排除连续换行）', 'recommendedLevel': 4},
                    {'key': _('空格(space)'), 'value': '(?<! ) (?! )', 'description': '匹配单个空格（排除前后有连续空格的情况）'},
                    {'key': _('分号(semicolon)'), 'value': '(?<!；)；(?!；)', 'description': '匹配单独的中文分号（排除连续分号如；；）'}, 
                    {'key': _('逗号(comma)'), 'value': '(?<!，)，(?!，)', 'description': '匹配单独的中文逗号（排除连续逗号如，，）'},
                    {'key': _('句号(period)'), 'value': '(?<!。)。(?!。)', 'description': '匹配单独的中文句号（排除连续句号如。。）'},
                    {'key': '-', 'value': '(?<! )- .*', 'description': '匹配无前导空格的列表项（如- 项目）'},   
                    {'key': "#", 'value': '(?<=^)# .*|(?<=\\n)# .*', 'description': '匹配Markdown一级标题（行首或换行后的单个#符号开头，如# 标题）'},
                    {'key': '##', 'value': '(?<=\\n)(?<!#)## (?!#).*|(?<=^)(?<!#)## (?!#).*', 'description': '匹配Markdown二级标题（行首或换行后的##符号开头，前后无相邻#）'},
                    {'key': '###', 'value': "(?<=\\n)(?<!#)### (?!#).*|(?<=^)(?<!#)### (?!#).*", 'description': '匹配Markdown三级标题（行首或换行后的###符号开头，前后无相邻#）'},
                    {'key': '####', 'value': "(?<=\\n)(?<!#)#### (?!#).*|(?<=^)(?<!#)#### (?!#).*", 'description': '匹配Markdown四级标题（行首或换行后的####符号开头，前后无相邻#）'},
                    {'key': '#####', 'value': "(?<=\\n)(?<!#)##### (?!#).*|(?<=^)(?<!#)##### (?!#).*", 'description': '匹配Markdown五级标题（行首或换行后的#####符号开头，前后无相邻#）'},
                    {'key': '######', 'value': "(?<=\\n)(?<!#)###### (?!#).*|(?<=^)(?<!#)###### (?!#).*", 'description': '匹配Markdown六级标题（行首或换行后的######符号开头，前后无相邻#）'}
                ] 
    ...
    class Batch(ApiMixin, serializers.Serializer):
        ...
        def batch_save(self, instance_list: List[Dict], with_valid=True):
            ...
            for document in instance_list:
                ...
                chunk_model_list.extend(document_paragraph_dict_model.get('chunk_model_list'))
        ...
        def batch_delete(self, instance: Dict, with_valid=True):
            ...
            QuerySet(Chunk).filter(document_id__in=document_id_list).delete()
        ...
        @transaction.atomic
        def batch_delete_by_name(self, names, with_valid=True):
            docs = Document.objects.filter(name__in=names).all()
            document_id_list = [doc.id for doc in docs]
            QuerySet(Document).filter(id__in=document_id_list).delete()
            QuerySet(Paragraph).filter(document_id__in=document_id_list).delete()
            QuerySet(Chunk).filter(document_id__in=document_id_list).delete()
            delete_problems_and_mappings(document_id_list)
