...
from dataset.serializers.chunk_serializers import create_paragraph_chunks, create_content_chunks
from embedding.models import SourceType
from embedding.task.embedding import delete_embedding_by_document_list, embedding_by_problem as embedding_by_problem_task, embedding_by_problem, \
...
class ParagraphSerializers(ApiMixin, serializers.Serializer):
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def edit(self, instance: Dict):
            ...
            if 'regenerateChunk' in instance and instance.get('regenerateChunk') is True:
                regenerateChunk = True

            if regenerateChunk:
                QuerySet(Chunk).filter(paragraph_id=self.data.get("paragraph_id")).delete()
                delete_embedding_by_paragraph(self.data.get('paragraph_id'))
                # 使用新的分块函数
                create_paragraph_chunks(
                    paragraph_content=_paragraph.content,
                    paragraph_id=self.data.get("paragraph_id"),
                    dataset_id=self.data.get('dataset_id'),
                    document_id=self.data.get('document_id')
                )
    ...
    class Create(ApiMixin, serializers.Serializer):
        ...
        def get_paragraph_problem_model(dataset_id: str, document_id: str, instance: Dict):
            ...
            chunk_list = instance.get('child_chunks', [])
            ...
            chunk_model_list = [Chunk(id=uuid.uuid1(),
                                paragraph_id=paragraph.id,
                                dataset_id=dataset_id,
                                document_id=document_id,
                                content=chunk.get('content') or chunk.get('text', ''),
                                char_length=len(chunk.get('text', '')),
                                chunk_index=index) for index, chunk in enumerate(chunk_list)]
