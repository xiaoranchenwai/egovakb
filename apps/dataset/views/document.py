...
from dataset.views.common import (get_dataset_document_operation_object, 
                                 get_dataset_operation_object,
                                 get_document_operation_object_batch, 
                                 get_document_operation_object)
from dataset.serializers.chunk_serializers import create_content_chunks
    ...
    @log(menu='document', operate="Import tables and create documents",
         get_operation_object=lambda r, keywords: get_dataset_document_operation_object(
             ...
             {'name': f'[{",".join([file.name for file in r.FILES.getlist("file")])}]',
              'document_list': [{'name': file.name} for file in r.FILES.getlist("file")]}))
    def post(self, request: Request, dataset_id: str):
        dataset = DataSetSerializers.Operate(data={'id': dataset_id}).one(
            user_id=request.user.id, role=request.user.role)
        documents_data = DocumentSerializers.Create(data={'dataset_id': dataset_id}).save_table(
            {'file_list': request.FILES.getlist('file')},
            request, dataset, with_valid=True)
        ret = result.success(documents_data)
        return ret
...
class Document(APIView):
    ...
    class Split(APIView):
        ...
        def post(self, request: Request):
            ...
            upload_type = request.data.get('uploadType')
            ...
            chunk_patterns = None
            chunk_length = None
            if 'chunk_patterns' in request.data:
                chunk_patterns = request_data.get('chunk_patterns')
            if 'chunk_length' in request.data:
                chunk_length = int(request_data.get('chunk_length'))
            ds = DocumentSerializers.Split(
                data=split_data)
            ds.is_valid(raise_exception=True)
            parsed_data = ds.parse()
            for paragraph in parsed_data:
                for content in paragraph['content']:
                    # 使用新的分块函数
                    chunk_list = create_content_chunks(
                    ...
                    for chunk in chunk_list:
                        content['child_chunks'].append(chunk)

            if 'documentUrls' in request.data:
                document_urls = request.data.getlist('documentUrls')
                if len(document_urls) == len(parsed_data):
                    for i in range(len(parsed_data)):
                        parsed_data[i]['document_url'] = document_urls[i]
            dataset_id = request.data.get('datasetId')
            cover = request.data.get('cover', 'false').lower() == 'true'
            if cover:
                names = [doc['name'] for doc in parsed_data]
                if len(names) > 0:
                    DocumentSerializers.Batch(data={'dataset_id': dataset_id}).batch_delete_by_name(names)
            if upload_type == DatasetUploadConsts.SIMPLE.value:                
                for item in parsed_data:
                    item['paragraphs'] = item['content']
                    del item['content']         

                documents_data = DocumentSerializers.Batch(data={'dataset_id': dataset_id}).batch_save(parsed_data)
                ret = result.success(documents_data)
                dataset = DataSetSerializers.Operate(data={'id': dataset_id}).one(
                    user_id=request.user.id, role=request.user.role)
