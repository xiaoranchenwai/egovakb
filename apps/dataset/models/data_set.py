...
class DataSet(AppModelMixin):
    ...
    question_model = models.ForeignKey(Model, related_name='question_model_id', 
                                       on_delete=models.SET_NULL, db_constraint=False,
                                       blank=True, null=True, verbose_name="问题生成模型")
...
class Document(AppModelMixin):
    ...
    meta = models.JSONField(verbose_name="元数据", default=dict)
    document_url = models.CharField(max_length=250, default="", verbose_name="文档地址")
...
class Chunk(AppModelMixin):
    """
    段落分块表
    """
    id = models.UUIDField(primary_key=True, max_length=128, default=uuid.uuid1, editable=False, verbose_name="主键id")
    paragraph = models.ForeignKey(Paragraph, on_delete=models.DO_NOTHING, db_constraint=False, verbose_name="所属段落")
    document = models.ForeignKey(Document, on_delete=models.DO_NOTHING, db_constraint=False, verbose_name="所属文档")
    dataset = models.ForeignKey(DataSet, on_delete=models.DO_NOTHING, verbose_name="所属数据集")
    content = models.CharField(max_length=1024, verbose_name="分块内容")
    chunk_index = models.IntegerField(verbose_name="分块序号", default=0)
