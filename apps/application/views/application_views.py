...
from common.permissions.group_permissions import filter_by_user_groups
...
class Application(APIView):
    ...
    def get(self, request: Request):
        return result.success(
            ApplicationSerializer.Query(
                data={**query_params_to_single_dict(request.query_params), 'user_id': request.user.id}).list(request.user.role))
    ...
    class Operate(APIView):
        ...
        def put(self, request: Request, application_id: str):
            return result.success(
                ApplicationSerializer.Operate(
                    data={'application_id': application_id, 'user_id': request.user.id, 'role': request.user.role}).edit(
                    request.data))
    ...
    class ListApplicationDataSet(APIView):
        ...
        def get(self, request: Request, application_id: str):
            return result.success(ApplicationSerializer.Operate(
                data={'application_id': application_id, 'user_id': request.user.id, 'role': request.user.role}).list_dataset())
        ...
        def get(self, request: Request, current_page: int, page_size: int):
            return result.success(
                ApplicationSerializer.Query(
                    ...
                    current_page, page_size, request.user.role))
    ...
    class Setting(APIView):
        ...
        @swagger_auto_schema(
            operation_summary=_("Get application settings"),
            operation_id=_("Get application settings"),
            tags=[_('Application/Settings')],
            responses=result.get_api_response(ApplicationApi.Setting.get_response_body_api())
        )
        @has_permissions(ViewPermission(
            [RoleConstants.ADMIN, RoleConstants.USER],
            [lambda r, keywords: Permission(group=Group.APPLICATION, operate=Operate.USE,
        ...
        def get(self, request: Request, application_id: str):
            return result.success(
                ApplicationSerializer.SettingSerializer().get_setting(application_id))

        @swagger_auto_schema(
            operation_summary=_("Update application settings"),
            operation_id=_("Update application settings"),
            tags=[_('Application/Settings')],
            request_body=ApplicationApi.Setting.get_request_body_api(),
            responses=result.get_api_response(ApplicationApi.Setting.get_response_body_api())
        )
        @has_permissions(ViewPermission(
            [RoleConstants.ADMIN, RoleConstants.USER],
        ...
        def put(self, request: Request, application_id: str):
            return result.success(
                ApplicationSerializer.SettingSerializer().update_setting(
