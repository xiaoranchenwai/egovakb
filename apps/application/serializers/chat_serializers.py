...
from application.models import Chat, Application, ApplicationDatasetMapping, VoteChoices, ChatRecord, WorkFlowVersion, \
    ApplicationTypeChoices, ApplicationApiKey
...
class ChatSerializers(serializers.Serializer):
    ...
    class Query(serializers.Serializer):
        ...
        api_key_id = serializers.UUIDField(required=False, error_messages=ErrMessage.uuid(_("API Key ID")))
        ...
        def get_query_set(self, select_ids=None):
            ...
            query_set = QuerySet(model=get_dynamics_model(
                {'application_chat.application_id': models.CharField(),
                 ...
                 'application_chat.api_key_id': models.CharField()}))
            ...
            if 'api_key_id' in self.data and self.data.get('api_key_id') is not None:
                base_query_dict['application_chat.api_key_id'] = self.data.get('api_key_id')
    ...
    class OpenChat(serializers.Serializer):
        ...
        api_key = serializers.CharField(required=False, allow_null=True, allow_blank=True,
                                         error_messages=ErrMessage.char(_("API Key")))
        ...
        def open(self):
            ...
            self.application_api_key = QuerySet(ApplicationApiKey).filter(application_id=application_id, secret_key=self.data.get('api_key')).first()
        ...
        def open_work_flow(self, application):
            ...
            api_key = self.application_api_key if self.application_api_key is not None else None
            chat_cache.set(chat_id,
                           ChatInfo(chat_id, [],
                                    [],
                                    application, work_flow_version, api_key), timeout=60 * 30)
            return chat_id
                           ...
                           ChatInfo(chat_id, dataset_id_list,
                                    [str(document.id) for document in
                                     QuerySet(Document).filter(
                                         ...
                                         is_active=False)],
                                    application, None, api_key), timeout=60 * 30)
            return chat_id
            ...
            if 'search_step' in chat_record.details and chat_record.details.get('search_step').get(
                ...
                dataset_list = [{'id': dataset_id, 'name': name} for dataset_id, name in reduce(lambda x, y: {**x, **y},
                                                                                                [{row.get(
                                                                                                    ...
                                                                                                    paragraph_list],
                                                                                                {}).items()]
                paragraph_document_ids = [row.get('document_id') for row in paragraph_list]
                document_lists = {
                    str(document.id): document
                    for document in QuerySet(Document).filter(id__in=paragraph_document_ids)
                }
                for paragraph in paragraph_list:
                    if str(paragraph.get('document_id')) in document_lists:
                        paragraph['document_url'] = document_lists.get(str(paragraph.get('document_id'))).document_url
