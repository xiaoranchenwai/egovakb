...
from application.models import Application, ApplicationDatasetMapping, ApplicationTypeChoices, WorkFlowVersion, ApplicationApiKey
...
from setting.models.group import GroupDetails, Group
...
def valid_model_params_setting(model_id, model_params_setting):
    ...
    credential.get_model_params_setting_form(
        model.model_name).valid_form(model_params_setting)
...
class ApplicationWorkflowSerializer(serializers.Serializer):
    ...
    def to_application_model(user_id: str, application: Dict):
        ...
        if application.get('work_flow') is not None:
            ...
            if application.get('dataset_id_list') is not None and len(application.get('dataset_id_list')) > 0:
                workflow_file_path = os.path.join(PROJECT_DIR, "apps", "application", 'flow',
                                                  f'default_workflow_zh_v1.json')
            default_workflow_json = get_file_content(workflow_file_path)
            default_workflow = json.loads(default_workflow_json)
        application['default_workflow'] = default_workflow
        for node in default_workflow.get('nodes'):
            ...
            if node.get('type') == 'ai-chat-node' and application.get('model_id') is not None:
                node.get('properties')['node_data']['model_id'] = application.get('model_id')
...
def get_base_node_work_flow(work_flow):
    ...
    base_node_list = [
        node for node in node_list if node.get('id') == 'base-node']
...
class ApplicationSerializer(serializers.Serializer):
    ...
    class Embed(serializers.Serializer):
        ...
        def get_embed(self, with_valid=True, params=None):
            ...
            is_draggable = 'false'
            show_guide = 'true'
            float_icon = f"{self.data.get('protocol')}://{self.data.get('host')}/ui/AiRobot.gif"
            xpack_cache = DBModelManage.get_model('xpack_cache')
            X_PACK_LICENSE_IS_VALID = False if xpack_cache is None else xpack_cache.get(
            ...
            application_setting = QuerySet(ApplicationSetting).filter(
            ...
            if application_setting is not None:
                ...
                if application_setting.float_location is not None:
                    ...
                    if isinstance(float_location, str):
                        float_location = json.loads(float_location)
                if isinstance(application_setting.custom_theme, str) and len(application_setting.custom_theme) > 0:
                    application_setting.custom_theme = json.loads(application_setting.custom_theme)
    ...
    class AccessTokenSerializer(serializers.Serializer):
        ...
        class AccessTokenEditSerializer(serializers.Serializer):
            ...
            avatar = UploadedImageField(required=False, error_messages=ErrMessage.image(_("Avatar")))
            float_icon = UploadedImageField(required=False, error_messages=ErrMessage.image(_("Float Icon")))
            user_avatar = UploadedImageField(required=False, error_messages=ErrMessage.image(_("User Avatar")))
        ...
        def edit(self, instance: Dict, with_valid=True):
            ...
            # 处理图片上传
            image_fields = ['avatar', 'float_icon', 'user_avatar']
            for field in image_fields:
                if field in instance and instance[field] is not None:
                    # 如果是文件对象,则保存新图片
                    if hasattr(instance[field], 'read'):
                        image_id = uuid.uuid1()
                        image = Image(id=image_id,
                                     image=instance[field].read(),
                                     image_name=instance[field].name)
                        image.save()
                        # 设置对应的 URL 字段
                        setattr(application_access_token, f'{field}', f'/api/image/{image_id}')
                    # 如果是字符串,直接设置URL
            ...
            if 'is_active' in instance:
                application_access_token.is_active = bool(instance.get("is_active"))
            if 'access_token_reset' in instance and instance.get('access_token_reset'):
                ...
                application_access_token.access_token = hashlib.md5(
                    str(uuid.uuid1()).encode()).hexdigest()[8:24]
            ...
            if 'white_active' in instance and instance.get('white_active') is not None:
                application_access_token.white_active = bool(instance.get("white_active"))
            ...
            if 'show_source' in instance and instance.get('show_source') is not None:
                application_access_token.show_source = bool(instance.get('show_source'))
            if 'language' in instance:
                application_access_token.language = instance.get('language')

            # 更新新增的字段
            fields_to_update = [
                'show_history', 'draggable', 'show_guide', 
                'disclaimer', 'disclaimer_value',
                'show_avatar', 'show_user_avatar'
            ]
            ...
            for field in fields_to_update:
                if field in instance:
                    ...
                    # 对布尔字段进行特殊处理
                    if field in ['disclaimer', 'show_history', 'draggable', 'show_guide']:
                        value = bool(value) if value is not None else None
                    setattr(application_access_token, field, value)
                
            # 处理特殊字段
            if 'custom_theme' in instance:
                application_access_token.custom_theme = instance.get('custom_theme')
                
            if 'float_location' in instance:
                application_access_token.float_location = instance.get('float_location')

            application_access_token.save()

            # 处理认证相关
            application_setting_model = DBModelManage.get_model(
                'application_setting')
            xpack_cache = DBModelManage.get_model('xpack_cache')
            X_PACK_LICENSE_IS_VALID = False if xpack_cache is None else xpack_cache.get(
                "XPACK_LICENSE_IS_VALID", False)
            if application_setting_model is not None and X_PACK_LICENSE_IS_VALID:
                ...
                if application_setting is not None and instance.get('authentication') is not None and instance.get(
                    ...
                    application_setting.authentication = instance.get(
                        'authentication')
            ...
            get_application_access_token(
                application_access_token.access_token, False)
        ...
        def one(self, with_valid=True):
            ...
            if application_access_token is None:
                application_access_token = ApplicationAccessToken(application_id=self.data.get('application_id'),
        ...
        def auth(self, request, with_valid=True):
            ...
            if application_access_token is not None and application_access_token.is_active:
                ...
                if authentication_value is not None:
                    # 认证用户token
                    self.auth_authentication_value(authentication_value, str(
    ...
    class Create(serializers.Serializer):
        ...
        def insert_workflow(self, application: Dict):
            self.is_valid(raise_exception=True)
            user_id = self.data.get('user_id')
            ApplicationWorkflowSerializer(
                data=application).is_valid(raise_exception=True)
            application_model = ApplicationWorkflowSerializer.to_application_model(
                user_id, application)
            application_model.save()
            # 插入认证信息
            token = hashlib.md5(str(uuid.uuid1()).encode()).hexdigest()[8:24]
            ApplicationAccessToken(application_id=application_model.id,
                                   access_token=token).save()
            ApplicationSetting(application_id=application_model.id,
                   show_source=True,
                   show_history=True,
                   draggable=False,
                   show_guide=True,
        ...
        def to_application_model(user_id: str, application: Dict):
            return Application(id=uuid.uuid1(), name=application.get('name'), desc=application.get('desc'),
                               ...
                               dialogue_number=application.get(
                                   'dialogue_number', 0),
                               user_id=user_id, model_id=application.get(
                                   'model_id'),
    ...
    class HitTest(serializers.Serializer):
        ...
        def hit_test(self):
            ...
            hit_list = vector.hit_test(self.data.get('query_text'), dataset_id_list, exclude_document_id_list,
                                       ...
                                       SearchMode(
                                           self.data.get('search_mode')),
                                           model)
            hit_dict = reduce(lambda x, y: {
                              **x, **y}, [{hit.get('paragraph_id'): hit} for hit in hit_list], {})
    ...
    class Query(serializers.Serializer):
        ...
        group_id = serializers.UUIDField(required=False, error_messages=ErrMessage.uuid(_("Group ID")))

        def get_query_set(self):
            user_id = self.data.get("user_id")
            role = self.data.get("role", None)
            is_admin = role == RoleConstants.ADMIN.name if role else False
            ...
            # 如果不是管理员或者明确指定了用户ID，则按用户ID过滤
            if not is_admin and 'select_user_id' in self.data and self.data.get('select_user_id') is not None and self.data.get('select_user_id') != 'all':
                query_set = query_set.filter(
                    **{'temp_application.user_id__exact': self.data.get('select_user_id')})
            
            query_set = query_set.order_by("-temp_application.create_time")
            query_set_dict['default_sql'] = query_set

            # 管理员可以查看所有应用，普通用户只能查看自己的
            if not is_admin:
                query_set_dict['application_custom_sql'] = QuerySet(model=get_dynamics_model(
                    {'application.user_id': models.CharField()})).filter(
        ...
        def list(self, role: str, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            app_list = []
            if role == RoleConstants.ADMIN.name:
                ...
                app_list = [ApplicationSerializer.Query.reset_application(a) for a in
                        native_search(qsd, select_string=get_file_content(
                            os.path.join(PROJECT_DIR, "apps", "application", 'sql', 'list_application_admin.sql')))]
            else:
                app_list = [ApplicationSerializer.Query.reset_application(a) for a in
            ...
            app_list = self.add_user_info(app_list)
            app_list = self.get_app_groups(app_list)
            group_id = self.data.get('group_id')
            if group_id:
                group_app_list = []
                for app in app_list:
                    if str(app.get('group_id')) == group_id:
                        group_app_list.append(app)
                return group_app_list
            return app_list

        @staticmethod
        def reset_application(application: Dict):
            application['multiple_rounds_dialogue'] = True if application.get(
                'dialogue_number') > 0 else False
        ...
        def add_user_info(app_list):
            ...
            # 获取所有用户ID
            user_ids = [app.get('user_id') for app in app_list]
            ...
            for app in app_list:
                ...
                if str(user_id) in users:
                    app['username'] = users[str(user_id)].username
                if str(user_id) in user_settings:
                    app['user_avatar'] = user_settings[str(user_id)].value
                    
            return app_list
            
        @staticmethod
        def get_app_groups(app_list):
            """
            根据应用ID查询应用所属的分组
            ...
            # 查询应用所属的分组详情
            group_details_dict = {
                str(detail.target_id): detail
                for detail in GroupDetails.objects.filter(
                target_type='APPLICATION',
                target_id__in=app_ids)
            }
            for app in app_list:
                if group_details_dict.get(str(app.get('id'))) is not None:
                    app['group_id'] = group_details_dict.get(str(app.get('id'))).group.id
            
            group_ids = [detail.group.id for detail in group_details_dict.values()]
            group_dict = {
                str(group.id): group
            ...
            for app in app_list:
                if group_dict.get(str(app.get('group_id'))) is not None:
                    app['group_name'] = group_dict.get(str(app.get('group_id'))).name
                
            return app_list

        def page(self, current_page: int, page_size: int, role: str, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            apps = []
            if role == RoleConstants.ADMIN.name:
                ...
                # 获取分页数据
                apps = native_page_search(current_page, page_size, qsd, 
                    select_string=get_file_content(
                        os.path.join(PROJECT_DIR, "apps", "application", 'sql', 'list_application_admin.sql')),
                    post_records_handler=ApplicationSerializer.Query.reset_application)                
            else:
                apps = native_page_search(current_page, page_size, self.get_query_set(), select_string=get_file_content(
                    os.path.join(PROJECT_DIR, "apps", "application", 'sql', 'list_application.sql')),
                    post_records_handler=ApplicationSerializer.Query.reset_application)
            
            # 使用新函数添加用户信息
            if apps.get('records'):
                apps['records'] = self.add_user_info(apps['records'])
                apps['records'] = self.get_app_groups(apps['records'])
    ...
    class ApplicationModel(serializers.ModelSerializer):
        class Meta:
            model = Application
            fields = ['id', 'name', 'desc', 'prologue',
                      'dialogue_number', 'icon', 'type']
    ...
    class IconOperate(serializers.Serializer):
        ...
        def edit(self, with_valid=True):
            ...
            application = QuerySet(Application).filter(
                id=self.data.get('application_id')).first()
            if application is None:
                raise AppApiException(500, _('Application id does not exist'))
            image_id = uuid.uuid1()
            image = Image(id=image_id, image=self.data.get(
                'image').read(), image_name=self.data.get('image').name)
            image.save()
    ...
    class Operate(serializers.Serializer):
        application_id = serializers.UUIDField(required=True, error_messages=ErrMessage.uuid(_("Application ID")))
        user_id = serializers.UUIDField(required=True, error_messages=ErrMessage.uuid(_("User ID")))
        role = serializers.CharField(required=False, error_messages=ErrMessage.char(_("Role")))
        ...
        def delete(self, with_valid=True):
            ...
            QuerySet(Application).filter(
                id=self.data.get('application_id')).delete()
            return True
        ...
        def one(self, with_valid=True):
            ...
            dataset_id_list = [d.get('id') for d in
                               list(filter(lambda row: mapping_dataset_id_list.__contains__(row.get('id')),
                                           dataset_list))]
            self.update_search_node(application.work_flow, [str(
                dataset.get('id')) for dataset in dataset_list])
            return {**ApplicationSerializer.Query.reset_application(ApplicationSerializerModel(application).data),
        ...
        def update_search_node(self, work_flow, user_dataset_id_list: List):
            ...
            for search_node in search_node_list:
                node_data = search_node.get(
                    'properties', {}).get('node_data', {})
            ...
            for search_node in search_node_list:
                ...
                for dataset_id in dataset_id_list:
                    if not user_dataset_id_list.__contains__(dataset_id):
                        # 将lazy_format对象转换为字符串
                        message = str(lazy_format(_('Unknown knowledge base id {dataset_id}, unable to associate'),
                ...
                result_dataset_id_list = [
                    *source_dataset_id_list, *result_dataset_id_list]
        ...
        def profile(self, with_valid=True):
            ...
            if X_PACK_LICENSE_IS_VALID:
                application_setting = QuerySet(ApplicationSetting).filter(
                    application_id=application_access_token.application_id).first()
                if application_setting is not None:
                    custom_theme = getattr(
                        application_setting, 'custom_theme', {})
                    float_location = getattr(
                    ...
                    if not custom_theme:
                        ...
                        if isinstance(custom_theme, str):
                            application_setting.custom_theme = json.loads(custom_theme)
                    if not float_location:
                        ...
                        if isinstance(float_location, str):
                            application_setting.float_location = json.loads(float_location)
                    application_setting_dict = {'show_source': application_setting.show_source,
            ...
            return ApplicationSerializer.Query.reset_application(
                {**ApplicationSerializer.ApplicationModel(application).data,
                 ...
                 'work_flow': application.work_flow,
                 'show_source': application_setting.show_source,
                 'language': application_setting.language,
                 **application_setting_dict})
        ...
        def edit(self, instance: Dict, with_valid=True):
            ...
            if 'work_flow' in instance:
                ...
                self.update_reverse_search_node(instance.get(
                    'work_flow'), application_dataset_id_list)
                # 找到语音配置相关
            ...
            for update_key in update_keys:
                if update_key in instance and instance.get(update_key) is not None:
                    application.__setattr__(
                        update_key, instance.get(update_key))
            application.save()
        ...
        def save_application_mapping(application_dataset_id_list, dataset_id_list, application_id):
            # 需要排除已删除的数据集
            dataset_id_list = [dataset.id for dataset in QuerySet(
                DataSet).filter(id__in=dataset_id_list)]
            # 删除已经关联的id
        ...
        def list_dataset(self, with_valid=True):
            ...
            if role is not None and role == RoleConstants.ADMIN.name:
                # 使用 values() 方法将 QuerySet 转换为字典列表
                dataset_list = QuerySet(DataSet).all().values()
                # 将 QuerySet 转换为 list
                return list(dataset_list)
                
            application = QuerySet(Application).get(
                id=self.data.get("application_id"))
    ...
    class ApplicationKeySerializer(serializers.Serializer):
        ...
        def generate(self, with_valid=True):
            ...
            application = QuerySet(Application).filter(
                id=application_id).first()
            hash_key = hashlib.md5(str(uuid.uuid1()).encode()).hexdigest()
            secret_key = 'application-' + hash_key
            name = hash_key[0:10]
            ...
            def edit(self, instance, with_valid=True):
                ...
                if 'cross_domain_list' in instance and instance.get('cross_domain_list') is not None:
                    application_api_key.cross_domain_list = instance.get(
                        'cross_domain_list')
                if 'name' in instance and instance.get('name') is not None:
                    application_api_key.name = instance.get('name')
                application_api_key.save()
                # 写入缓存
                get_application_api_key(application_api_key.secret_key, False)

    class SettingSerializer(serializers.Serializer):
        show_history = serializers.BooleanField(required=False, 
            error_messages=ErrMessage.boolean(_("Show History")))
        draggable = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Draggable")))
        show_guide = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Show Guide")))
        avatar = UploadedImageField(required=False, allow_null=True,
            error_messages=ErrMessage.image(_("AI Avatar")))
        float_icon = UploadedImageField(required=False, allow_null=True,
            error_messages=ErrMessage.image(_("Float Icon")))
        user_avatar = UploadedImageField(required=False, allow_null=True,
            error_messages=ErrMessage.image(_("User Avatar")))
        disclaimer = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Show Disclaimer")))
        disclaimer_value = serializers.CharField(required=False, allow_null=True,
            error_messages=ErrMessage.char(_("Disclaimer Content")))
        custom_theme = serializers.JSONField(required=False,
            error_messages=ErrMessage.json(_("Custom Theme")))
        float_location = serializers.JSONField(required=False,
            error_messages=ErrMessage.json(_("Float Location")))
        authentication = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Enable Authentication")))
        authentication_value = serializers.JSONField(required=False,
            error_messages=ErrMessage.json(_("Authentication Settings")))
        access_token = serializers.CharField(required=False, allow_null=True,
            error_messages=ErrMessage.char(_("Access Token")))
        access_num = serializers.IntegerField(required=False, min_value=0,
            error_messages=ErrMessage.integer(_("Access Count")))
        white_active = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Enable Whitelist")))
        white_list = serializers.ListField(required=False,
            child=serializers.CharField(),
            error_messages=ErrMessage.list(_("Whitelist")))
        show_source = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Show Source")))
        language = serializers.CharField(required=False, allow_null=True,
            error_messages=ErrMessage.char(_("Language")))
        show_avatar = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Show Avatar")))
        show_user_avatar = serializers.BooleanField(required=False,
            error_messages=ErrMessage.boolean(_("Show User Avatar")))


        def get_setting(self, application_id):
            """获取应用设置"""
            application_setting = QuerySet(ApplicationSetting).filter(
                application_id=application_id).first()
            ...
            return {
                ...
                'show_guide': application_setting.show_guide,
                'avatar': application_setting.avatar,
                'avatar_url': application_setting.avatar_url,
                'float_icon': application_setting.float_icon,
                'float_icon_url': application_setting.float_icon_url,
                'user_avatar': application_setting.user_avatar,
                'user_avatar_url': application_setting.user_avatar_url,
                'disclaimer': application_setting.disclaimer,
                'disclaimer_value': application_setting.disclaimer_value,
                'show_avatar': application_setting.show_avatar,
                'show_user_avatar': application_setting.show_user_avatar,
                'custom_theme': json.loads(application_setting.custom_theme) if len(application_setting.custom_theme) > 0 else None,
                'float_location': application_setting.float_location if len(application_setting.float_location) > 0 else None,
                'is_active': application_access_token.is_active,
                'authentication': application_setting.authentication,
                'authentication_value': application_setting.authentication_value if len(application_setting.authentication_value) > 0 else None,
                'access_token': application_setting.access_token,
                'access_num': application_setting.access_num,
                'white_active': application_setting.white_active,
                'white_list': application_setting.white_list,
                'show_source': application_setting.show_source,
                'language': application_setting.language
            }

        def update_setting(self, application_id, instance):
            """更新应用设置"""
            ...
            application_access_token = QuerySet(ApplicationAccessToken).filter(
                application_id=application_id).first()
            if application_access_token != None:
                setattr(application_setting, 'access_token', application_access_token.access_token)
            # 处理图片文件上传
            image_fields = ['avatar', 'float_icon', 'user_avatar']
            for field in image_fields:
                if field in instance:
                    field_value = instance[field]
                    if field_value is not None:  # 只在值不为 None 时更新
                        # 如果是文件对象,则保存新图片
                        if hasattr(field_value, 'read'):
                            # 处理新上传的图片
                            image_id = uuid.uuid1()
                            image = Image(id=image_id,
                                        image=field_value.read(),
                                        image_name=field_value.name)
                            image.save()
                            # 设置图片URL
                            setattr(application_setting, field, f'/api/image/{image_id}')
                            setattr(application_setting, f'{field}_url', f'/api/image/{image_id}')
                        elif isinstance(field_value, str):
                            continue

            # 处理布尔类型字段
            bool_fields = ['show_history', 'draggable', 'show_guide', 'disclaimer', 'show_source', 'white_active', 'show_avatar', 'show_user_avatar']
            for field in bool_fields:
                if field in instance and instance[field] is not None:  # 只在值不为 None 时更新
                    value = instance[field]
                    if isinstance(value, str):
                        # 将字符串转换为布尔值
                        value = value.lower() == 'true'
                    setattr(application_setting, field, value)
            ...
            for field in other_fields:
                if field in instance and instance[field] is not None:  # 只在值不为 None 时更新
                    setattr(application_setting, field, instance[field])

            application_setting.save()
            return self.get_setting(application_id)
