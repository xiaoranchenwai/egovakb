SELECT
	application_chat.*,
	application_api_key.name as api_key_name,
	application_api_key.id as api_key_id,
	to_json(asker) as asker
FROM
	application_chat application_chat
	LEFT JOIN application_api_key ON application_chat.api_key_id = application_api_key.id
	LEFT JOIN (
	SELECT COUNT
