...
class UserSerializer(ApiMixin, serializers.ModelSerializer):
    ...
    def listByType(self, type, user_id, role):
        targets = []
        ...
        if type == 'DATASET':
            if role != RoleConstants.ADMIN.name:
                ...
                user_list = DataSet.objects.all().prefetch_related(prefetch_users).distinct('user_id')
            ...
            if role != RoleConstants.ADMIN.name:
                ...
                user_list = Application.objects.all().prefetch_related(prefetch_users).distinct('user_id')
        elif type == 'FUNCTION':
            if role != RoleConstants.ADMIN.name:
                ...
                user_list = FunctionLib.objects.all().prefetch_related(prefetch_users).distinct('user_id')
