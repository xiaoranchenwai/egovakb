...
def chunk_data(data: Dict, chunk_patterns: str = None, chunk_length: int = None):
    if str(data.get('source_type')) == SourceType.PARAGRAPH.value:
        text = data.get('text')
        chunk_list = text_to_chunk(text, chunk_patterns, chunk_length)
        return [{**data, 'text': chunk} for chunk in chunk_list]
    return [data]


def chunk_data_list(data_list: List[Dict], chunk_patterns: str = None, chunk_length: int = None):
    result = [chunk_data(data, chunk_patterns, chunk_length) for data in data_list]
...
class BaseVectorStore(ABC):
    ...
    def batch_save_chunk(self, chunk_list, embedding: Embeddings, is_the_task_interrupted):
        self.save_pre_handler()
        result = sub_array(chunk_list)
        for child_array in result:
            if not is_the_task_interrupted():
