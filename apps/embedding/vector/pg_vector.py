...
class PGVector(BaseVectorStore):
    ...
    def _batch_save(self, text_list: List[Dict], embedding: Embeddings, is_the_task_interrupted):
        """
        批量保存向量数据。
        注意：对传入的文本进行空值过滤，避免后端嵌入模型报错（如“【vector text】该字段不能为空”）。
        仅对 text 非空且去除空白后仍有内容的记录执行嵌入与写入。
        """
        # 过滤空文本记录，防止嵌入报错
        filtered_list = [row for row in text_list if row.get('text') is not None and str(row.get('text')).strip() != ""]
        if len(filtered_list) == 0:
            max_kb.error(_('Batch save vector data: No valid text records to process.'))
            return True
        texts = [row.get('text') for row in filtered_list]
        embeddings = embedding.embed_documents(texts)
        embedding_list = [Embedding(id=uuid.uuid1(),
                                    document_id=filtered_list[index].get('document_id'),
                                    paragraph_id=filtered_list[index].get('paragraph_id'),
                                    dataset_id=filtered_list[index].get('dataset_id'),
                                    is_active=filtered_list[index].get('is_active', True),
                                    source_id=filtered_list[index].get('source_id'),
                                    source_type=filtered_list[index].get('source_type'),
                                    embedding=embeddings[index],
                                    chunk_id=filtered_list[index].get('chunk_id'),
                                    search_vector=SearchVector(Value(to_ts_vector(filtered_list[index]['text'])))) for
                          index in
                          range(0, len(texts))]
    ...
    def delete_by_chunk_id(self, chunk_id: str):
        QuerySet(Embedding).filter(chunk_id=chunk_id).delete()
