...
def get_embedding_model(model_id, exception_handler=lambda e: max_kb.error(
...
@celery_app.task(base=QueueOnce, once={'keys': ['chunk_id']}, name='celery:embedding_by_chunk')
def embedding_by_chunk(chunk_id, model_id):
    embedding_model = get_embedding_model(model_id)
    ListenerManagement.embedding_by_chunk(chunk_id, embedding_model)
...
def embedding_by_document(document_id, model_id, state_list=None):
    ...
    :return: None
    """
    max_kb.info(_('celery task embedding_by_document: {document_id}').format(document_id=document_id))
