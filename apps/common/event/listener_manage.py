...
from dataset.models.data_set import Chunk
...
class ListenerManagement:
    ...
    def embedding_by_paragraph_data_list(data_list, paragraph_id_list, embedding_model: Embeddings):
        max_kb.info(_('Start--->Embedding paragraph list: {paragraph_id_list}').format(
        ...
        try:
            ...
            QuerySet(Paragraph).filter(
                id__in=paragraph_id_list).update(**{'status': status})
            max_kb.info(
                _('End--->Embedding paragraph: {paragraph_id_list}').format(paragraph_id_list=paragraph_id_list))

    @staticmethod
    def embedding_by_chunk(chunk_id, embedding_model: Embeddings):
        """
        向量化分块 根据分块id
        @param chunk_id:    分块id
        @param embedding_model:  向量模型
        说明：过滤空文本分块，避免嵌入模型报错（【vector text】该字段不能为空）。
        """
        max_kb.info(_('Start--->Embedding chunk: {chunk_id}')
                    .format(chunk_id=chunk_id))
        try:
            chunk = QuerySet(Chunk).filter(id=chunk_id).first()
            if chunk is None:
                max_kb.error(_('Chunk id {chunk_id} does not exist').format(chunk_id=chunk_id))
                return
            # 空文本保护：chunk 内容为空则跳过
            if chunk.content is None or str(chunk.content).strip() == "":
                max_kb_error.error(_('Chunk content is empty: {chunk_id}').format(chunk_id=chunk_id))
                return
            # 删除分块
            VectorStore.get_embedding_vector().delete_by_chunk_id(chunk_id)
            chunk_list = []
            chunk_list.append({
                'id': chunk.id,
            ...
            def is_the_task_interrupted():
                return False
            if len(chunk_list) > 0:
                VectorStore.get_embedding_vector().batch_save_chunk(
                    chunk_list, embedding_model, is_the_task_interrupted)
        except Exception as e:
            max_kb_error.error(_('Vectorized chunk: {chunk_id} error {error} {traceback}').format(
                chunk_id=chunk_id, error=str(e), traceback=traceback.format_exc()))
        finally:
            max_kb.info(
                _('End--->Embedding chunk: {chunk_id}').format(chunk_id=chunk_id))
    ...
    def embedding_by_paragraph(paragraph_id, embedding_model: Embeddings):
        ...
        @param embedding_model:  向量模型
        说明：优先向量化非空分块；若分块全部为空，则回退到段落文本。过滤空或仅空白文本以避免嵌入后端报错。
        """
        max_kb.info(_('Start--->Embedding paragraph: {paragraph_id}')
                    .format(paragraph_id=paragraph_id))
        # 更新到开始状态
        ListenerManagement.update_status(QuerySet(Paragraph).filter(
            id=paragraph_id), TaskType.EMBEDDING, State.STARTED)
        try:
            data_list = native_search(
                {'problem': QuerySet(get_dynamics_model({'paragraph.id': django.db.models.CharField()})).filter(
            ...
            max_kb.info(
                _('End--->Embedding paragraph: {paragraph_id}').format(paragraph_id=paragraph_id))
    ...
    def get_aggregation_document_status(document_id):
        def aggregation_document_status():
            ...
            native_update({'document_custom_sql': QuerySet(Document).filter(
                id=document_id)}, sql, with_table_name=True)
    ...
    def get_aggregation_document_status_by_query_set(queryset):
        def aggregation_document_status():
            sql = get_file_content(
                os.path.join(PROJECT_DIR, "apps", "dataset", 'sql', 'update_document_status_meta.sql'))
            native_update({'document_custom_sql': queryset},
                          sql, with_table_name=True)
    ...
    def post_update_document_status(document_id, task_type: TaskType):
        ...
        if _document.status_meta is None:
            max_kb.info(f"document_id:{document_id} status_meta is None")
            ...
            for item in _document.status_meta.get('aggs', []):
                ...
                if Status(agg_status)[task_type] == State.FAILURE and agg_count > 0:
                    status[task_type] = State.FAILURE
        ListenerManagement.update_status(QuerySet(Document).filter(
            id=document_id), task_type, status[task_type])
    ...
    def embedding_by_document(document_id, embedding_model: Embeddings, state_list=None):
        ...
        :return: None
        """
        max_kb.info(f"celery task ListenerManagement embedding_by_document document:{document_id}")
        if state_list is None:
            state_list = [State.PENDING, State.SUCCESS,
                          State.FAILURE, State.REVOKE, State.REVOKED]
        if not try_lock('embedding' + str(document_id)):
            max_kb.info(_('Document: {document_id} is being processed, skip embedding.').format(document_id=document_id))
            return
        try:
            def is_the_task_interrupted():
            ...
            max_kb.info(_('End--->Embedding document: {document_id}')
    ...
    def embedding_by_dataset(dataset_id, embedding_model: Embeddings):
        ...
        try:
            ...
            max_kb.info(
                _('Start--->Embedding document: {document_list}').format(document_list=document_list))
            for document in document_list:
                max_kb.info(
                    _('Start--->Embedding document: {document}').format(document=document))
                print(document)
            ...
            max_kb.info(
                _('End--->Embedding dataset: {dataset_id}').format(dataset_id=dataset_id))
    ...
    @staticmethod
    def delete_embedding_by_chunk(chunk_id):
        VectorStore.get_embedding_vector().delete_by_chunk_id(chunk_id)
    ...
    def update_problem(args: UpdateProblemArgs):
        problem_paragraph_mapping_list = QuerySet(
            ProblemParagraphMapping).filter(problem_id=args.problem_id)
