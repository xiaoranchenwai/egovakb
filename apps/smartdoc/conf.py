...
class Config(dict):
    defaults = {
        ...
        'MCP_SSE_SERVER': '',
        "BACKEND_API_PREFIX": '',
        "FRONTEND_PATH_PREFIX": '/ui/',
        # SkyWalking 配置
        "SKYWALKING_COLLECTOR": "",
        "SKYWALKING_SERVICE_NAME": "大模型智能应用平台",
        "SKYWALKING_GROUP": "",
    }
    ...
    def get_db_setting(self) -> dict:
        pool_options = {
            "POOL_SIZE": int(self.get('DB_POOL_SIZE', 30)),
            "MAX_OVERFLOW": int(self.get('DB_MAX_OVERFLOW', 100)),
            "POOL_TIMEOUT": int(self.get('DB_POOL_TIMEOUT', 30)),
            "POOL_RECYCLE": int(self.get('DB_POOL_RECYCLE', 3600)),
            "POOL_PRE_PING": self.get('DB_POOL_PRE_PING', True),
            "POOL_RESET_ON_RETURN": self.get(
